<template>
  <div class="sticky top-0 z-10 flex-shrink-0 flex h-16 bg-white shadow">
    <button type="button" class="px-4 border-r border-gray-200 text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-emerald-500 md:hidden" @click="sidebarOpen = true">
      ...
      <div class="flex-1 flex">
        <form class="w-full flex md:ml-0" @submit.prevent>
          ...
          <div class="relative w-full text-gray-400 focus-within:text-gray-600">
            ...
            <input id="search-field" class="cursor-pointer block w-full h-full pl-8 pr-3 py-2 border-transparent text-gray-900 font-semibold placeholder-gray-500 placeholder:font-semibold focus:outline-none focus:placeholder-gray-400 focus:ring-0 focus:border-transparent sm:text-sm" @click.prevent="search" placeholder="Search anything..." readonly type="search" autocomplete="off" name="search" />
          </div>
        </form>
      </div>
      <div class="ml-4 flex items-center md:ml-6">
        <button type="button" @click="openNotificationsWindow" class="bg-white p-1 rounded-full text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
        ...
        <Menu as="div" class="ml-3 relative">
          <div>
            <MenuButton class="max-w-xs bg-white flex items-center text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
              <span class="sr-only">Open user menu</span>
              <div class="h-8 w-8 rounded-full bg-emerald-100 flex justify-center items-center text-sm font-semibold">
          ...
          <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <MenuItems class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
              ...
              <a :href="item.href" :class="[currentRouteName === item.routeName ? 'active-link-color bg-gray-100' : 'text-gray-800', 'block px-4 py-2 text-sm']">
              ...
              <div @click="handleSignout" class="w-full cursor-pointer">
                Sign out
              </div>
              </MenuItem>
            </MenuItems>
  ...
  <DynamicModal :type="typeModal" :id="idModal" :gridColumnAmount="gridColumnModal" :title="titleModal" :description="descriptionModal" :firstButtonText="firstButtonModal" :secondButtonText="secondButtonModal" :thirdButtonText="thirdButtonModal" :open="openModal" @firstModalButtonFunction="firstModalButtonFunction" @secondModalButtonFunction="secondModalButtonFunction" @thirdModalButtonFunction="thirdModalButtonFunction">
    ...
    <div v-if="signOutFormForModal">
      ...
      <main>
        <form ref="signOutForm" method="post" action="/logout" class="myPrimaryForm">
  ...
  <SearchAnythingModal :open="openSearchModal" @toggleSearchModal="openSearchModal = !openSearchModal">
  </SearchAnythingModal>

  <SlideOverNotifications :open="notificationsWindow" @closeNotificationsWindow="closeNotificationsWindow">
...
import { BellIcon, MenuAlt2Icon, CogIcon } from '@heroicons/vue/outline'
