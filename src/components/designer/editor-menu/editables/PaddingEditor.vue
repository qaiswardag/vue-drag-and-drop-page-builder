            ...
            <div class="my-2">
                ...
                <div>
                    <select
                        ...
                        <option
                            v-for="padding in TAILWIND_NUMBER_PROPERTIES" :key="padding">
                            {{ padding }}
                        </option>
                    </select>
                </div>
...
const getCurrentElementOuterHTML = computed(() => {
    return getCurrentElement?.value?.outerHTML ? getCurrentElement?.value?.outerHTML : []
})
//
...
watch(getCurrentElementOuterHTML, (newElement, oldElement) => {
    ...
    const yPaddingClassExist = Array.from(newElement.classList).some(singleClass => {
    ...
    const xPaddingClassExist = Array.from(newElement.classList).some(singleClass => {
