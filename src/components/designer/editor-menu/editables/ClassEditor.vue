    ...
    <EditorAccordion>
        ...
        <template #content>
            ...
            <div class="flex flex-row flex-wrap gap-2 mt-2 mb-4">
                <p v-for="className in currentClasses" :key="className"
                   class="inline-block bg-emerald-500 text-white hover:bg-emerald-700 text-xs px-3 py-1 cursor-pointer rounded-2xl duration-200"
            ...
            <p :class="[containsSpace ? 'max-h-5' : 'max-h-0']"
               class="text-red-500 text-sm mb-4 overflow-hidden duration-200">Class may not contain
                spaces*</p>
        </template>
    </EditorAccordion>
</template>
...
const addClass = function (e) {
    if (inputClass.value !== '' && !containsSpace.value && !inputClass.value.includes(' ') && (e.keyCode === 13 || e.type === 'click')) {
